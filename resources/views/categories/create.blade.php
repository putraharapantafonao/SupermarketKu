<x-app-layout>
    <x-slot name="header">
        <div class="space-y-1">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Tambah Kategori</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Buat kategori produk baru untuk kelompok barang SupermarketKu</p>
        </div>
    </x-slot>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 sm:p-6">
            ...
            <form action="{{ route('categories.store') }}" method="POST" class="space-y-5">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Makanan, Minuman, Kosmetik..."
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea name="description" rows="4" placeholder="Penjelasan singkat mengenai kelompok kategori barang ini..."
                              class="w-full border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white rounded-xl text-sm focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 focus:ring-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="text-xs text-red-500 mt-1">❌ {{ $message }}</span>
                    @enderror
                </div>
