<x-app-layout>
    <x-slot name="header">
        <div class="space-y-1">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Supplier</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui informasi data pemasok barang SupermarketKu</p>
        </div>
    </x-slot>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 sm:p-6">
            ...
            <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" class="space-y-5">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="text" name="name" value="{{ old('name', $supplier->name) }}" placeholder="Contoh: PT. Sumber Rezeki, CV. Maju Jaya..."
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" placeholder="Contoh: 081234567xxx..."
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea name="address" rows="4" placeholder="Tulis alamat operasional kantor atau gudang supplier..."
                              class="w-full border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white rounded-xl text-sm focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 focus:ring-red-500 @enderror">{{ old('address', $supplier->address) }}</textarea>
                    @error('address')
                        <span class="text-xs text-red-500 mt-1">❌ {{ $message }}</span>
                    @enderror
                </div>
