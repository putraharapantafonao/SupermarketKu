<x-app-layout>
    <x-slot name="header">
        <div class="space-y-1">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Data Supplier</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Kelola data pemasok barang SupermarketKu</p>
        </div>
    </x-slot>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-gray-600 dark:text-gray-300">
                        @foreach ($suppliers as $supplier)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition-colors">
                                ...
                                <td class="px-5 py-4 text-sm max-w-xs truncate" title="{{ $supplier->address }}">
                                    {{ $supplier->address ?? '-' }}
                                </td>
                                <td class="px-5 py-4 text-sm text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" class="m-0 inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="return confirm('Yakin hapus supplier {{ $supplier->name }}?')"
